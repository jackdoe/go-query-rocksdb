...
func NewRocksIndexWithDB(db *gorocksdb.DB, perField map[string]*analyzer.Analyzer) *RocksIndex {
	return &RocksIndex{TotalNumberOfDocs: 1, rocks: db, perField: perField}
}
func NewRocksIndex(root string, perField map[string]*analyzer.Analyzer) (*RocksIndex, error) {
	...
	return NewRocksIndexWithDB(db, perField), nil
}
